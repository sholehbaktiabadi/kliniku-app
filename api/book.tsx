...
export const getBookOrderSummary = async (data: { session: string, sequence: string, polyClinicId: string }) => {
    const response = await axios.get(env.baseUrl.klinikuApi + `/book/summary?sequence=${data.sequence}&polyClinicId=${data.polyClinicId}`, {
